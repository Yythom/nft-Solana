...
const PopContent = memo(({ text, content, hover, showHover, }: { text: any, content?: any, hover?: any, showHover?: boolean }) => {
    const { mode } = useSelector((state: any) => state.user, shallowEqual);
    ...
    return <Fragment>
        ...
        >
            ...
            {text && <Popover
                position='bottomLeft'
                content={
                    <div style={{ color: mode === 'dark' ? '#fff' : '#333', maxWidth: '400px', padding: '8px', wordBreak: 'break-all' }}>
                        {text}
                    </div>
                }>
